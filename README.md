...
On top of that base image, it will install redis-py and then take the contents of the directory ./azure-vote and copy it into the container in the path /app.
...
Note that the login-server will be on the format .azurecr.io and that Username will be the same as "your unique ACR name" used when creating the container registry.
...
If you want to, you can check if helm works by running the helm versioncommand:
